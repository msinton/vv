...
object UserDao {
  ...
  val users: MongoCollection[User] = mongoClient
    .getDatabase("vv")
    .withCodecRegistry(codecRegistry)
    .getCollection("users")

  private[db] val indexes: SingleObservable[String] = users.createIndexes(
    List(
  ...
  def get(username: String, email: String) =
    users
      .find(and(equal("username", username), equal("email", email)))
      .first()
      .toFuture()

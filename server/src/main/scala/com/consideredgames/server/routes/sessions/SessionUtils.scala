...
package object SessionUtils {
  ...
  def hasValidSession(username: String, sessionId: String, ip: String)(implicit ec: ExecutionContext): Future[Boolean] =
    SessionDao
      .getRecent(sessionId, username, ip, preparedSessionTimeout)
      .map(_ != null)

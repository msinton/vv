...
case class Animal(info: AnimalInfo, female: Boolean, var wild: Boolean, var pregnant: Option[Pregnant]) {
  ...
  def isPregnant: Boolean = pregnant.isDefined
}
...
object Animal {
  ...
  def serializer(animalInfos: List[AnimalInfo]): CustomSerializer[Animal] = {
    ...
    object serializer
        extends CustomSerializer[Animal](
          _ =>
            ({
              case JObject(List(JField(str, JString(n)))) if str == key =>
                ...
                if (splits.length == 3) {
                  Animal(
                    animalInfos
                      .find(_.name == splits(0))
                      .getOrElse(throw ResourceInvalidException(
                        s"Could not deserialize animal, as not an animal Info match $n")),
                    splits(1).toBoolean,
                    splits(2).toBoolean
                  )
                } else {
                  Animal(
                    ...
                    splits(1).toBoolean,
                    splits(2).toBoolean,
                    Option(Pregnant(splits(3).toInt))
                  )
                }
            }, {
              case x: Animal =>

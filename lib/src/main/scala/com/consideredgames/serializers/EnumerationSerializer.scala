...
class EnumerationSerializer[E <: Enumeration: ClassTag](enum: E, key: Option[String] = None)
  ...
  def deserialize(implicit format: Formats): PartialFunction[(TypeInfo, JValue), E#Value] =
    new PartialFunction[(TypeInfo, JValue), E#Value] {
      ...
      def apply(typeAndValue: (TypeInfo, JValue)): E#Value = typeAndValue match {
        case (TypeInfo(EnumerationClass, _), JObject(List(JField("enum", JString(e)), JField("value", JString(v)))))
            if e == enumKey => {
          enum.withName(v)
        }
        case (TypeInfo(EnumerationClass, _), v) =>
          throw new MappingException("Can't convert " + v + " to enum " + enum.getClass.getName)
  ...
  def serialize(implicit format: Formats): PartialFunction[Any, JValue] = new PartialFunction[Any, JValue] {
    ...
    def apply(x: Any): JValue = x match {
      ...
      case other: E#Value =>
        throw new IllegalArgumentException("can't serialize enum " + other + " of type " + getEnumerationClass(other))
      case other: AnyRef =>
        throw new IllegalArgumentException("can't serialize object " + other + " of type " + other.getClass.getName)
    }
  }

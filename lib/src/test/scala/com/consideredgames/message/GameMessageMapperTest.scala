...
class GameMessageMapperTest extends FunSuite with TryValues {
  ...
  test("PrincipalPhasePreProcess") {
    ...
    val m = PrincipalPhasePreProcess(
      ...
      List(
        PersonToActionResult(Person(1, Black),
                             ResourceResult(ResourceProduct(List(ResourceGroup(Wood, 1))), 2, Lumberjack))),
      List(),
      List()
    )
  ...
  test("PrincipalPhasePreProcess with animals killed") {
    ...
    val m = PrincipalPhasePreProcess(AttackResults(List(), Map()), List(), List(Animal(animalInfos(0), true)), List())

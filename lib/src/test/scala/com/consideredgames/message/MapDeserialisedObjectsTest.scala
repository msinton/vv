...
class MapDeserialisedObjectsTest extends FunSuite with OptionValues {
  ...
  test("maps person from list") {
    ...
    val princ =
      PrincipalAction(List(ActionFulfillment(actions.Fish, PersonActionParameter(Person(1, PlayerColours.Black)))),
                      assTools)
  ...
  test("maps person from list, two person actions") {
    ...
    val princ = PrincipalAction(
      List(
        ActionFulfillment(actions.Fish,
                          TwoPersonActionParameter(Person(1, PlayerColours.Black), Person(2, PlayerColours.Black)))),
      Map())
